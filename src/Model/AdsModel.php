...
class AdsModel
{
    ...
    */
    ...
    /**
     * Shows data for books after searching
     *
     * @access public
     * @param array $data books data
     * @return array Result
     */
   /* public function showAds($data)
    {
        if ($data!=null) {
            $ads=array();
            foreach ($data as $row) {
                $query = '
                  ...
                  WHERE
                    idad=?
                ';
                $books[]= $this->db->fetchAll($query, array($row['idad']));
            }
            $view['ad']=array();
            return $view['ad'];
     ...
     * @return array Result
     */
    /*public function searchAds($data)
    {
        if ($data!=null) {
            $value="^.*(".$data.").*$";
            ...
            $query = '
              ...
              WHERE
                ad_name REGEXP ?
            ';
            $result=$this->db->fetchAll($query, array($value));
            $query2 = '
              SELECT
                idad
              FROM
              ...
              WHERE
                ad_contence REGEXP ? ';
            $result2=$this->db->fetchAll($query2, array($value));
